...
const FormField = ({
  ...
  <div className="flex flex-col space-y-1">
    <label className="text-sm font-bold text-black mb-2 uppercase tracking-wide opacity-80">{label}</label>
    <div
      className={`flex items-center space-x-2 border-2 rounded-2xl p-2 bg-white transition duration-150 h-12 ${
    ...
    >
      ...
      <input
        ...
        className="w-full focus:outline-none text-sm text-gray-900 bg-transparent"
...
export const RegistrationForm = () => {
  ...
  return (
    <div className="max-w-xl w-full p-10 rounded-[2.5rem] bg-pink-50 shadow-xl relative">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-2 gap-x-8 gap-y-6">
        <div className="col-span-1 space-y-6">
          ...
          <FormField
            ...
            icon={<svg className="w-5 h-5 text-purple-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>}
          />
          <FormField
            label="CONTRASEÑA"
            name="contrasena"
        ...
        <div className="col-span-1 space-y-6">
          <FormField
            ...
            icon={<svg className="w-5 h-5 text-purple-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>}
          />
          <FormField
            label="TELÉFONO"
            name="telefono"
          ...
          <FormField
            ...
            register={register}
            error={errors.email}
            icon={<svg className="w-5 h-5 text-purple-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>}
          />

          <div className="pt-2">
            <label className="text-xs text-gray-700 font-semibold block mb-2">SELECCIONAR TIPO DE USUARIO</label>
            <Controller
                  ...
                  {userTypes.map((type) => (
                    <label key={type} className="flex items-center space-x-2 cursor-pointer">
                      <input
                        ...
                        className="hidden"
                      />
                      <div className="w-5 h-5 border-2 rounded-full flex items-center justify-center transition duration-150 ease-in-out"
                           style={{ borderColor: field.value === type ? '#9333ea' : '#d1d5db' }}>
                        <div className="w-2 h-2 rounded-full"
                             style={{ backgroundColor: field.value === type ? '#9333ea' : 'transparent' }}>
                        </div>
                      </div>
            ...
            />
            {errors.tipoUsuario?.message && <p className="text-xs text-red-500 ml-3 mt-1">{errors.tipoUsuario.message}</p>}
          </div>
        </div>

        <button
          type="submit"
          className={`col-span-2 w-64 mx-auto mt-4 h-12 rounded-2xl text-white font-bold shadow-lg bg-purple-600 border-0 hover:opacity-90 transition-all text-lg`}
        >
          REGISTRAR
        </button>
      </form>
