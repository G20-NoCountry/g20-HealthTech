...
const profileSchema = z.object({
  nombreCompleto: z.string().trim().min(3, 'Debe tener al menos 3 caracteres.').regex(nameRegex, 'Solo letras y espacios.'),
  ...
  telefono: z.string().min(7, 'Mínimo 7 dígitos.').max(15, 'Máximo 15 dígitos.').regex(/^\d+$/, 'Solo números.'),
                  ...
                  render={({ field }) => (
                    <div className={`flex items-center border-2 rounded-[2rem] p-4 bg-white transition duration-150 h-20 ${errors.fechaNacimiento ? 'border-red-500' : 'border-gray-200 hover:border-purple-400'}`}>
                      <Calendar
                        ...
                        className="w-full text-2xl bg-transparent focus:outline-none border-0 shadow-none"
                        pt={{
                          input: { className: 'text-2xl bg-transparent border-0 shadow-none focus:outline-none w-full' },
                          dropdownButton: { className: 'text-purple-200' },
                        }}
                      />
                    </div>
                  )}
